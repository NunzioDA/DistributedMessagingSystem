...
import threading
import socket
from communication_API.elite_servers_API import DISCOVERY_MSG, ENROLL_MSG
from tools.dms_secure import *
import random
...
class EliteServer:
    ...
    def discover_request(self, client_socket, username, server_id):
        with open(self.servers_file_path, 'r') as servers_file:
            ...
            user_hash = hash_to_range(username, len(clusters))

            if(server_id !="None"):
                server_hash = hash_to_range(server_id, len(clusters))
